...
export default function SalaryPage() {
  ...
  useEffect(() => {
    employeeService.getAll({ limit: 50 }).then((r) => setEmployees(r.data.data || []));
  }, []);
  ...
  const openModal = (newRecord = false) => {
    setIsNew(newRecord);
    reset(salary ? {
      basicSalary: salary.basicSalary, hra: salary.hra,
      travelAllowance: salary.travelAllowance, medicalAllowance: salary.medicalAllowance,
      specialAllowance: salary.specialAllowance, providentFund: salary.providentFund,
      professionalTax: salary.professionalTax, incomeTax: salary.incomeTax,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (isNew) {
        await salaryService.create({ ...data, employee: selected._id });
      } else {
    ...
    <div className="flex flex-col h-full">
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden flex flex-col">
            <div className="p-4 border-b border-border">
              <div className="relative">
                ...
                <Input placeholder="Search employee…" className="pl-9" value={search}
                  onChange={(e) => setSearch(e.target.value)} />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto divide-y divide-border/50">
              {filtered.map((emp) => (
                <button key={emp._id} onClick={() => loadSalary(emp)}
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{emp.user?.name}</p>
            ...
            {!selected ? (
              <div className="h-full flex items-center justify-center rounded-xl border border-dashed border-border">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Select an employee to view salary</p>
                </div>
              </div>
            ) : loading ? (
              ...
              <div className="h-full flex items-center justify-center rounded-xl border border-dashed border-border">
                <div className="text-center space-y-3">
                  <p className="text-sm text-muted-foreground">No salary structure found for {selected.user?.name}</p>
                  <Button size="sm" onClick={() => openModal(true)}>Create Salary Structure</Button>
                </div>
              </div>
            ) : (
              <div className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                  <div>
                    <h3 className="text-sm font-semibold text-foreground">{selected.user?.name}</h3>
                    <p className="text-xs text-muted-foreground">Effective from {formatDate(salary.effectiveFrom)}</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => openModal(false)} className="gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        { label: "Travel Allowance", value: salary.travelAllowance },
                      ...
                      <div className="border-t border-border pt-2 flex justify-between text-sm font-semibold">
                        ...
                        <span className="text-green-600 dark:text-green-400">{formatCurrency(salary.grossSalary)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div key={item.label} className="flex justify-between text-sm">
                          <span className="text-muted-foreground">{item.label}</span>
                          <span className="font-medium text-red-600 dark:text-red-400">-{formatCurrency(item.value)}</span>
                        </div>
                      ))}
                      <div className="border-t border-border pt-2 flex justify-between text-sm font-semibold">
                    ...
                    <div className="mt-4 rounded-lg bg-primary/5 border border-primary/20 p-4 flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-primary">{formatCurrency(salary.netSalary)}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{isNew ? "Create" : "Edit"} Salary Structure</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
            <div>
              <Label>Effective From</Label>
              <Input type="date" {...register("effectiveFrom")} className="mt-1.5" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={field.name}>
                  ...
                  <Input type="number" {...register(field.name as keyof FormData)} className="mt-1" placeholder="0" />
                </div>
              ))}
            </div>
            <DialogFooter>
